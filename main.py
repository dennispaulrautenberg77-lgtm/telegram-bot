...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[
        InlineKeyboardButton("🔄 Exchange starten",
                             callback_data='start_exchange')
    ], [
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'start_exchange':
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT user_id, final FROM orders WHERE id=?",
                       (order_id, ))
        row = cursor.fetchone()
        ...
        if row:
            ...
            if action == 'accept':
                ...
                await context.bot.send_message(
                    target_user_id,
                    f"✅ Deine Anfrage wurde vom Admin **akzeptiert**. Die Auszahlung von {final_amount}€ in LTC erfolgt in Kürze! 🚀"
                )
                await query.edit_message_text(
                    f"✅ Du hast die Anfrage #{order_id} **akzeptiert**.")
            else:
                cursor.execute(
                ...
                await context.bot.send_message(
                    target_user_id,
                    "❌ Deine Anfrage wurde vom Admin **abgelehnt**. Bei Fragen wende dich an den Support."
                )
                await query.edit_message_text(
                    f"❌ Du hast die Anfrage #{order_id} **abgelehnt**.")
        conn.commit()
        conn.close()
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current_state == STATE_WAITING_LTC:
        ...
        try:
            ...
            await update.message.reply_text(
                "❌ Bitte gib einen gültigen Betrag ein (nur Zahl).")
        ...
        if update.message.photo or update.message.document:
            ...
            """, (user_id, update.effective_user.username,
                  state_data['ltc_address'], state_data['psc_code'],
                  state_data['amount'], state_data['fee'], state_data['final'],
                  'PENDING'))
            ...
            keyboard = [[
                InlineKeyboardButton("✅ Akzeptieren",
            ...
            await update.message.reply_text(
                "⚠️ Bitte sende einen Screenshot (Foto) vom Bon.")
